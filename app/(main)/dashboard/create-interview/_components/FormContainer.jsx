...
function FormContainer({ onHandleInputChange, GoToNext }) {
  ...
  const AddInterviewType = (type) => {
    // If the clicked type is already selected, deselect it; otherwise, set it as the main type.
    if (interviewType === type) {
      setInterviewType('')
    } else {
  ...
  return (
    <div className='p-5 bg-white rounded-xl'>
        ...
        <div className='flex gap-3 flex-contain mt-2'>
          {InterviewType.map((type, index) => (
            <div key={index} className={`flex items-center cursor-pointer gap-2 p-1 px-2 bg-white border border-gray-300 rounded-2xl 
              hover:bg-secondary ${interviewType === type.title && 'bg-blue-100 text-primary'}`} onClick={() => AddInterviewType(type.title)}>
      ...
      <div className='mt-7 flex justify-end'>
        <Button onClick={() => { GoToNext(interviewType) }}>
